...
public class CloudOnly {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String appId = "CloudOnly"; // identifier of the application
		    ...
		    application.addAppModule("datahost", 100);
			...
			for(int i=0;i<DCs.size();i++) {
				moduleMapping.addModuleToDevice("datahost", DCs.get(i).getName());
			...
			//adding edges from producers to chosen data hosts
				...
				for(int j=0;j<producers.size();j++) {
						 ...
						 application.addTupleMapping("datahost", "data"+(j+1), "data"+(j+1), new FractionalSelectivity(0.1));	
					   if(producers.get(j).getLevel() < datahosts.get(0).getLevel())	
					       application.addAppEdge("producer"+producers.get(j).getId(), "datahost", 20, 960, "data"+(j+1), Tuple.DOWN, AppEdge.MODULE);
					   else
						   application.addAppEdge("producer"+producers.get(j).getId(), "datahost", 20, 960, "data"+(j+1), Tuple.UP, AppEdge.MODULE);
				       
					  
					   
					   for(int k=0;k<temp_loops.size();k++) {
				    	   if(temp_loops.get(k).contains("producer"+producers.get(j).getId())) {
				    		   temp_loops.get(k).add("datahost");   		   
				    	   }
				       }
					}
				...
				for(int j=0;j<producers.size();j++) {
				   for(int k=0;k<consumers.size();k++) {
					   if(consumerToData[j][k]==1) {
						    	  ...
						    	  if(datahosts.get(0).getLevel() < consumers.get(k).getLevel())
						    	      application.addAppEdge("datahost", "consumer"+consumers.get(k).getId(), 20, 960, "data"+(j+1), Tuple.DOWN, AppEdge.MODULE);
							      else
							    	  application.addAppEdge("datahost", "consumer"+consumers.get(k).getId(), 20, 960, "data"+(j+1), Tuple.UP, AppEdge.MODULE);
							      
							      for(int l=0;l<temp_loops.size();l++) {
					                 if(temp_loops.get(l).contains("datahost")) {
