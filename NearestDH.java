...
public class NearestDH {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String appId = "NearestDH"; // identifier of the application
			...
			//Building the storage cost matrix
			int storageTime[][]=new int[producers.size()][datahosts.size()];
			for(int i=0;i<producers.size();i++) {
				for(int j=0;j<datahosts.size();j++) {
					storageTime[i][j]=getLatency(producers.get(i),datahosts.get(j));
				}
			}
			System.out.println("Storage cost built ");
			
			//Building the consumer to data item consumed matrix
			...
			int solution[][]=new int[datahosts.size()][producers.size()];
			for(int i=0;i<solution.length;i++) {
				...
				for(int j=0;j<producers.size();j++) {
					...
					application.addTupleMapping("datahost"+datahosts.get(pos).getId(), "data"+(j+1), "data"+(j+1), new FractionalSelectivity(0.1));	
					   if(producers.get(j).getLevel() < datahosts.get(pos).getLevel())	
					       application.addAppEdge("producer"+producers.get(j).getId(), "datahost"+datahosts.get(pos).getId(), 20, 960, "data"+(j+1), Tuple.DOWN, AppEdge.MODULE);
					   else
						   application.addAppEdge("producer"+producers.get(j).getId(), "datahost"+datahosts.get(pos).getId(), 20, 960, "data"+(j+1), Tuple.UP, AppEdge.MODULE);
				       
					  
					   
					   for(int k=0;k<temp_loops.size();k++) {
				    	   if(temp_loops.get(k).contains("producer"+producers.get(j).getId())) {
				    		   temp_loops.get(k).add("datahost"+datahosts.get(pos).getId());   		   
				    	   }
				       }
						   ...
						   for(int i=0;i<datahosts.size();i++) {
						       if(solution[i][j]==1) {
							      ...
							      for(int l=0;l<temp_loops.size();l++) {
					                 if(temp_loops.get(l).contains("datahost"+datahosts.get(i).getId())) {
					                	 ArrayList<String> copy=new ArrayList<>();
					                	 ArrayList<String> temp=temp_loops.get(l);
