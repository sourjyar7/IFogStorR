...
public class iFogStorA {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String appId = "iFogStorA"; // identifier of the application
			...
			//Building the retrieval cost matrix
			int retrievalTime[][]=new int[consumers.size()][datahosts.size()];
			for(int i=0;i<consumers.size();i++) {
				for(int j=0;j<datahosts.size();j++) {
					retrievalTime[i][j]=getLatency(consumers.get(i),datahosts.get(j));
				}
			}
			...
			int solution[][]=solver.gapSolver(solver);
					...
					if(solution[i][j]==1) {
					   ...
					   for(int k=0;k<temp_loops.size();k++) {
				    	   if(temp_loops.get(k).contains("producer"+producers.get(j).getId())) {
				    		   temp_loops.get(k).add("datahost"+datahosts.get(i).getId());   		   
				    	   }
				       }
					}
				}
